...
type TabType = "vacina" | "vermifugo" | "cio" | "tarefa";
...
export default function AgendaCaninaPage() {
  ...
  // Filter events based on active tab and dog filter (for the list view)
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      {viewMode === "calendar" ? (
        ...
        /* List view mode with categories */
        <>
          {/* Tabs and Filters */}
          <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4 border-b border-salon-border pb-4">
            ...
            <div className="flex flex-wrap gap-2">
              {(["vacina", "vermifugo", "cio", "tarefa"] as TabType[]).map((tab) => {
                const isActive = activeTab === tab;
                const label = tab === "vacina" 
                  ? "Vacinas" 
                  : tab === "vermifugo" 
                  ? "Vermífugos" 
                  : tab === "cio" 
          ...
          <div className="bg-salon-surface border border-salon-border rounded-salon p-6 w-full max-w-md space-y-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              ...
              <div className="space-y-1">
                ...
                <select
                  value={form.type}
                  onChange={(e) => setForm((prev) => ({ ...prev, type: e.target.value as TabType }))}
